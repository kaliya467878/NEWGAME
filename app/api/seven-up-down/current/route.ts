...
export async function GET() {
  try {
    ...
    // Deterministic seed for current round
    ...
    const result = resolveSevenUpDownRound();
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        status: remainingSeconds <= 5 ? "SHAKING" : "BETTING",
        bettingLocked: remainingSeconds <= 5,
        result: remainingSeconds <= 5 ? result : null,
      },
    });
  } catch (error: any) {
    console.error("GET 7up-down current API error:", error);
