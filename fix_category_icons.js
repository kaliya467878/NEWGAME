...
if (fs.existsSync(path)) {
  ...
  // Replace the entire renderCategoryIcon function
  const start = code.indexOf('const renderCategoryIcon');
  const end = code.indexOf('export default function CategoryTabs');
  
  if (start !== -1 && end !== -1) {
    const newIcons = `const renderCategoryIcon = (iconName) => {
  const svgProps = {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
...
`;
    code = code.substring(0, start) + newIcons + code.substring(end);
    fs.writeFileSync(path, code);
    console.log('Updated CategoryTabs.js icons');
  }
}
